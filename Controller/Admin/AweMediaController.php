...
class AweMediaController extends AbstractController
{
    const SJIS = 'sjis-win';
    const UTF = 'UTF-8';
    private $errors = [];
    ...
    public function index(Request $request)
    {
        ...
        if (!$isAjax) {
            throw $this->createNotFoundException('This is not ajax Request.');
        }
        ...
        $isTopDir = $this->checkDir( $topDir , $nowDir );
    ...
    private function getFileList($nowDir)
    {
        ...
        $fileFinder = $finder->files()->name('/\.(jpg|png|jpeg|webp|gif)$/');
        try {
